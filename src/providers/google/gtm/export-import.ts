...
/** The GTM object collections this codebase models, extracted from a container export. */
 ...
 * container export (Admin → Export Container). `builtInVariable` is part of the
 * format but not modeled here — ignored, not errored on.
 */
export function parseContainerExport(json: unknown): ContainerExport {
  ...
  return { folder: collection('folder'), variable: collection('variable'), trigger: collection('trigger'), tag: collection('tag') };
}
...
/**
 * Turns a parsed container export into the `{ folders, variables, triggers, tags, skipped }`
 * shape used by `AnalyticsConfig['gtm']`, reverse-mapping each raw GTM object via `fromGtmPayload`.
 * Folders are processed first so other kinds can resolve `parentFolderId` to `folder`; triggers
 * before tags so tags can resolve `firingTriggerId`/`blockingTriggerId` to `trigger`/`exceptTrigger`.
 *
...
export function resourcesFromExport(container: ContainerExport): Record<string, unknown> {
  ...
  const usedFolderSlugs = new Set<string>();
  const folderGtmIdToLogicalId: Record<string, string> = {};
  const folders: Record<string, unknown> = {};
  for (const object of container.folder) {
    try {
      const slug = slugify(String(object.name ?? ''), usedFolderSlugs);
      folders[slug] = fromGtmPayload('folder', object);
      const folderId = object.folderId;
      if (typeof folderId === 'string') folderGtmIdToLogicalId[folderId] = slug;
    } catch {
      skipped++;
    }
  }
  ...
  for (const object of container.trigger) {
    try {
      ...
      triggers[slug] = fromGtmPayload('trigger', object, { folderGtmIdToLogicalId });
  ...
  for (const object of container.variable) {
    try {
      variables[slugify(String(object.name ?? ''), usedVariableSlugs)] = fromGtmPayload('variable', object, { folderGtmIdToLogicalId });
    } catch {
      skipped++;
    }
  }
  ...
  for (const object of container.tag) {
    try {
      tags[slugify(String(object.name ?? ''), usedTagSlugs)] = fromGtmPayload('tag', object, { triggerGtmIdToLogicalId, folderGtmIdToLogicalId });
    } catch {
      skipped++;
    }
  }

  return { folders, variables, triggers, tags, skipped };
}
