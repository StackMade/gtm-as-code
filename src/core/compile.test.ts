...
test('more than 50 event-scoped custom dimensions (hand-written + event-derived) throws', () => {
  ...
  const config = baseConfig({ ga4: { dimensions, metrics: {}, keyEvents: {}, audiences: {}, eventCreateRules: {}, eventEditRules: {} } });
...
test('derived id colliding with an unrelated explicit resource in another section throws', () => {
  const config = baseConfig({
    ...
    ga4: { dimensions: { generate_lead: { scope: 'event', parameter: 'unrelated' } }, metrics: {}, keyEvents: {}, audiences: {}, eventCreateRules: {}, eventEditRules: {} },
  });
